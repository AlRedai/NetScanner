...
char *bannerservice(unsigned short bport) {
	...
	if((conn = connect(sock,(struct sockaddr *)&ban, sizeof(struct sockaddr))) == -1) 
		return ("N.B. error");
	if(bport == 80) 
		send(sock, HTTPHEAD, strlen(HTTPHEAD), 0);
	recv(sock, banner, 1000, 0);
	if(bport == 80)  {
		head = strstr(banner,"Server");
		if(head != NULL) {
			for(x=0; x<=strlen(head); x++) {
				if(head[x] == '\n') { 
					head[x] = '\0'; 
					break; 
				} 
			}
			wserver = (char *)malloc(x-8);
			for(y=0; y<=x; y++) {
				wserver[y] = head[y+8];
				if(head[y+8] == '\0') { 
					break; 
...
int main(int argc, char **argv) {
	...
	if (rangeport) {
		printf("    Host: %s  Method: %s  Port: [%u-%u]\n\n", hostname, method, min, max);
		if(!verbose) {
			printf("    ");
